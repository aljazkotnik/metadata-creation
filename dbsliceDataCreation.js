// ADD FUNCTIONALITY TO HANDLE A SINGLE METADATA FILE HORIZONTALLY. Maybe just a split within hte builder is enough. Alhough, the collection of the merger data will have to be careful about this too.

// THE BUILD.MAKE MUST TAKE IN THE FILEOBJS AS AN INPUT!! OTHERWISE LOADING EXISTING MERGE DATA IS TRICKY!!


// DONE: rework merging functionality
// DONE: UPDATE THE CATEGORY OF ITEMS AFTER THEY ARE MOVED!!
// DONE: ALLOW LOADING OF MERGING DATA. Just need to implement the appropriate file type, and a loading button.


// ADD IN SUPPORT FOR DRAGGING AND DROPPING DATA
// ALLOW THE USER TO NAME THE GROUPS INTERACTIVELY??
// ADD A TASK ID CATEGORY??



// MAYBE STILL TRY TO MATCH UP THE VARIABLES SOMEHOW? AT LEAST THE COMPLETE MATCHES??


// ALLOW FILES TO BE ARRANGED IN ORDER?
...
var dbsliceDataCreation = {
	...
	make: function(){
		...
		let allMergerInfo = fileManager.library.retrieve(mergerInfoFile)
		
		// Construct the appropriate internal data
		let fileobjs = dbsliceDataCreation.makeInternalData(allMetadataFiles)
		allMergerInfo.forEach(function(mergerInfo){
			fileobjs = dbsliceDataCreation.sortByLoadedMergingInfo(fileobjs, mergerInfo)
		}) // forEach
	...
	show: function show(){
		...
		let fullscreenContainer = d3.select("div.merging-container")
	...
	hide: function hide(){
		...
		d3.select("div.merging-container").style("display", "none")
	...
	submit: function submit(){
		...
		// Perform the actual merger
		
		let mergedData = dbsliceDataCreation.merge(mergerInfo)
		
		console.log("SUBMIT", mergerInfo, mergedData)
	...
	drag: {
		...
		helpers: {
			...
			isContainerCompatible: function(container, draggedDom){
				...
				return dbsliceDataCreation.catCompatibleTypes[category].includes(varType)
			...
			styling: {
				...
				adjust: function(){
					...
					let categories = d3.select("div.merging-container")
	...
	builder: {
		...
		build: {
			...
			header: function header(){
				...
				let types = dbsliceDataCreation.supportedCategories
			...
			footer: function(){
				...
				var mergeInfoInput = createFileInputElement( fileManager.importing.mergerinfo )
		...
		color: function color(accessor){
			...
			let scheme = d3.scaleOrdinal(d3.schemePastel2)
			  .domain(dbsliceDataCreation.supportedCategories)
	...
	operateOverCategories: function(categories, action){
		...
		dbsliceDataCreation.supportedCategories.forEach(function(categoryType){
	...
	makeInternalData: function (allMetadataFiles){
		...
		let cat2ind = dbsliceDataCreation.cat2ind
		let ind2cat = dbsliceDataCreation.ind2cat
	...
	merge: function(mergerInfo){
		...
		// The 'mergedData' can be tailored to fit better with 'cfDataManagement' later on.
		var tasks = []
		
		
		// Determine what filename_id would work for all files.
		...
		let header = {}
		Object.getOwnPropertyNames(mergerInfo).forEach(function(category){
			header[category] = Object.getOwnPropertyNames(mergerInfo[category])
		})
