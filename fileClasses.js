// WHAT HAPPENS IF THE FILE HAS TWO VARIABLES OF THE SAME NAME?? - d3.load only retains the last one. Just alert the user of this! THIS SHOULD BE IN THE FILE CLASSES ACTUALLY!!
	...
	*/
	...
	report: {
		...
		generate: function(){
			...
			report = errors.log.reduce(function(acc, er){
				if(acc[er.requester]){
...
class metadataFile extends dbsliceFile {
  ...
  static cat2var(categories){
	  ...
	  let supportedCategories = Object.getOwnPropertyNames(dbsliceDataCreation.cat2type)
	  let cat2type = dbsliceDataCreation.cat2type
...
class contour2dFile extends onDemandFile {
	...
	static structure = {
		...
		json2contour2dFile: function(content){
			...
			// Easiest would be to just attempt to use this to create a contour. This would likely add further complications...
			
			// The json file must include an array under the field 'surfaces', which contains one entry per surface.
			
			if(Array.isArray(content.surfaces) && content.surfaces.length > 0){
				...
				contour2dFile.test.content(content_)
	...
	static test = {
		...
		content: function(content){
			...
			// The surfaces have to have "x", "y", "v", and "size" attributes.
			let requiredProperties = ["x", "y", "v", "size"]
			
			let areSurfacesValid = content.surfaces.every(function(s){
				return helpers.arrayIncludesAll(Object.getOwnPropertyNames(s), requiredProperties)
			})
			...
			// Try to access the content in the expected way.
			return true
...
class userFile extends dbsliceFile {
	...
	format(obj){
		...
		let availableFileClasses = [metadataFile, mergerInfoFile]
...
class mergerInfoFile extends userFile {
	
	
	format(obj){
		
		obj.content = dbsliceFile.test.structure(mergerInfoFile, obj.content)
		return obj
	...
	static structure = {
		...
		json2mergerInfoFile: function(content){
			...
			// WHAT SHOULD THE INTERNAL STRUCTURE BE?? I GUESS AS IT IS IN MERGER_INFO.JSON. OR WOULD IT BE SIMPLER TO HAVE IT IN A CSV FORMAT?? JUST KEEP IT AS IT IS IN THE TEMPLATE FOR NOW.
			
			// Prune away anything that is not in line with the expected structure.
			content_ = dbsliceDataCreation.supportedCategories.map(function(category){
